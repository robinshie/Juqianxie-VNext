...
using IdentityService.Respository;
...
namespace IdentityService.Infrastructure
{
    class IdRepository : IIdRepository
    {
        ...
        public async Task<IdentityResult> RemoveUserAsync(int id)
        {
            ...
            //就会报错：The instance of entity type 'IdentityUserLogin<int>' cannot be tracked because another instance with the same key value for {'LoginProvider', 'ProviderKey'} is already being tracked.
        ...
        public async Task<User?> FindByHospitalNumberAsync(string hospitalNumber)
        {
            return await userManager.Users.FirstOrDefaultAsync(u => u.HospitalNumber == hospitalNumber);
        }

        public async Task<(IdentityResult, User?)> RegisterByHospitalNumberAsync(string hospitalNumber, string userName, string password)
        {
            if (password.Length < 6)
            {
                IdentityError err = new IdentityError();
                err.Code = "Password Invalid";
                err.Description = "密码长度不能少于6";
                return (IdentityResult.Failed(err), null);
            }
            if (await FindByHospitalNumberAsync(hospitalNumber) != null)
            {
                return (ErrorResult($"已经存在住院号{hospitalNumber}"), null);
            }
            User user = new User(userName, hospitalNumber);
            var result = await CreateAsync(user, password);
            if (!result.Succeeded)
            {
                return (result, null);
        ...
        public async Task<(IdentityResult, User?)> ChangePasswordAsync(string hospi, string password)
        {
            if (password.Length < 6)
            {
                IdentityError err = new IdentityError();
            ...
            {
                var user = await FindByHospitalNumberAsync(hospi);
                if (user == null)
                {
                    IdentityError err = new IdentityError();
                    err.Code = "404";
                    err.Description = "无此用户";
                    return (IdentityResult.Failed(err), null);
                }
